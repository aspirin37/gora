<template>
    <div>
        <main-page-head id="main"
                        v-on:scrollDown="scrollToItem('services', 500)"></main-page-head>
        <main-page-services id="services"
                            v-on:scrollDown="scrollToItem('projects', 500)"></main-page-services>
        <!-- Portfolio -->
        <div id="projects">
            <main-project-view v-for="(project, index) in portfolioItems"
                               :info="project"
                               :key="`project-${index}`"
                               :id="`project-${project.path}`"></main-project-view>
        </div>
        <!-- /Portfolio -->
        <section class="js-scroll-block main-section bg-light height-half"
                 id="team">
            <div class="container">
            ...
            <div class="container text-center">
                <router-link tag="button"
                             :to="{name: 'team'}"
                             class="btn btn-light">More about team</router-link>
            </div>
        </section>
        <section class="js-scroll-block main-section bg-dark height-half d-flex align-items-center"
                 id="contact">
            <div class="container">
        ...
        <nav class="side-nav d-none d-md-block">
            <a href="#main"
               ...
               v-on:click.prevent="scrollToItem('main', 500)"></a>
            <a href="#services"
               class="side-nav__link border d-block ml-auto"
               :class="{'bg-dark active': scrolledBlock === 'services'}"
               v-on:click.prevent="scrollToItem('services', 500)"><span class="side-nav__text">{{$t('titles.whatwedo')}}</span></a>
            <!-- Projects nav -->
            <a :href="`#project-${project.path}`"
               ...
               v-on:click.prevent="scrollToItem(`project-${project.path}`, 500)"><span class="side-nav__text">{{project.title}}</span></a>
            <!-- Projects nav -->
            <a href="#team"
               class="side-nav__link border d-block ml-auto"
               :class="{'bg-dark active': scrolledBlock === 'team'}"
               v-on:click.prevent="scrollToItem('team', 500)"><span class="side-nav__text">{{$t('titles.team')}}</span></a>
            <a href="#contact"
               class="side-nav__link border d-block ml-auto"
               :class="{'bg-dark active': scrolledBlock === 'contact'}"
               v-on:click.prevent="scrollToItem('contact', 500)"><span class="side-nav__text">{{$t('titles.contact')}}</span></a>
        </nav>
    </div>
</template>
<script>
